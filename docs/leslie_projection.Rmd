...
```{r}
# Make the Leslie Matrix that is in a 4x4 set-up by row so first 4 values are first row:
...
# making a vector from 0 to 8 incrementing by 1
time_yr <- seq(from = 0, to = 8, by = 1)

# making an empty matrix that will have columns of how many values are in fish year 0 (4) and rows of time in years (9)
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

#  the first row in the matrix should be populated by fish_year0
proj_year[1, ] <- fish_year0

# starting at row 2 and iterate over the length of time for the rest of the rows with the equation using the dot product of the fish_leslie matrix with the previous rows values of the matrix
...
# this added column names
colnames(proj_year) <- c("eggs", "fry", "juvenile", "adult")

# this creates a data frame, and move columns into long format where a column indicates the life stage and the second column will be the resulting value
